...
beerocks::eChannelScanErrCode dynamic_channel_selection_task::dcs_request_scan_dump()
{
    ...
    auto request = beerocks::message_com::create_vs_message<
        beerocks_message::cACTION_CONTROL_CHANNEL_DUMP_SCAN_RESULTS_REQUEST>(cmdu_tx);
    if (request == nullptr) {
        LOG(ERROR) << "Failed building message cACTION_CONTROL_CHANNEL_DUMP_SCAN_RESULTS_REQUEST";
