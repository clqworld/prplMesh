                                                     ...
                                                     const std::string &src_mac)
{
     ...
     * have defined two different TLVs: tlvLinkMetricQuerySpecificNeighbor with the optional
     * field and tlvLinkMetricQueryAllNeighbors without it. Application must check which of both
     * TLVs has been received inside the message.
     */
    ...
    if (!tlvLinkMetricQueryAllNeighbors) {
        ...
        if (!tlvLinkMetricQuerySpecificNeighbor) {
            LOG(ERROR) << "getClass ieee1905_1::tlvLinkMetricQueryAllNeighbors and "
                          "ieee1905_1::tlvLinkMetricQuerySpecificNeighbor failed";
            return false;
        }
    }
    ...
    if (tlvLinkMetricQuerySpecificNeighbor) {
        /**
    	   ...
    	   * permissive enough to allow it specify ALL_NEIGHBORS and if so, then we will just
    	   * ignore the field containing the MAC address of neighbor.
    	   */
    ...
    if (invalid_neighbor) {
        ...
        if ((network_utils::mac_to_string(neighbor_al_mac) == controller_bridge_mac) &&
            ...
            /**
             * Compute particular Ethernet media type
             */
