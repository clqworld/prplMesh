...
bool monitor_thread::handle_cmdu(Socket *sd, ieee1905_1::CmduMessageRx &cmdu_rx)
{
    ...
    case beerocks_message::ACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST: {
        auto request =
            beerocks_header
                ->addClass<beerocks_message::cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST>();
        if (!request) {
            LOG(ERROR) << "addClass cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST failed";
            return false;
        }

        auto response_out = message_com::create_vs_message<
            beerocks_message::cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE>(
            cmdu_tx, beerocks_header->id());
        if (!response_out) {
            LOG(ERROR)
                << "Failed building cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE message!";
            return false;
        }
