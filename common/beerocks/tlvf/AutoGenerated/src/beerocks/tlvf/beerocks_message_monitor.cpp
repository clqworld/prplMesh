...
cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST::cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST::cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST::~cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST() {
}
void cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST::class_swap()
{
}

bool cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST::get_initial_size()
{
    size_t class_size = 0;
    return class_size;
}

bool cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_REQUEST::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE::cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE::cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE::~cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE() {
}
uint8_t& cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE::success() {
    return (uint8_t&)(*m_success);
}

void cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE::class_swap()
{
}

bool cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(uint8_t); // success
    return class_size;
...
bool cACTION_MONITOR_CHANNEL_DUMP_SCAN_RESULTS_RESPONSE::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
